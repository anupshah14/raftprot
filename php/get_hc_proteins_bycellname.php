...
foreach($input as $checkbox){
	$query_parts[] = "'%".mysql_real_escape_string($checkbox)."%'";
}
//print_r($query_parts);
$string = implode(' OR experiment_new.idCellname like ', $query_parts);
//echo $string."\n";
$query=mysql_query("select protein_new.accession,count(protein_new.accession) as protein_no, protein_new.protein_name, protein_new.gene_name,high_confidence.lable from protein_new
							...
							where experiment_new.idExperiment in (select experiment_new.idExperiment from experiment_new where experiment_new.idCellname like $string) group by protein_new.accession") or die (mysql_error());	
//echo $query."\n";
$query_exp_id=mysql_query("select idExperiment from experiment_new where idCellname like $string group by idExperiment") or die (mysql_error());	

while ($exp_id=mysql_fetch_assoc($query_exp_id)){
	$data['exp_ids'][]=$exp_id;
//	echo count($data['exp_id']);
	}
