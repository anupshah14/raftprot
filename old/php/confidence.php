...
$data=array();
	...
	$query=mysql_query("select protein_new.accession, protein_new.gene_name, protein_new.protein_name, count(distinct experiment_new.biochemMethod,experiment_new.detergent) as biochemCount,count(distinct protein_new.Experiment_idExperiment) as protein_count,subcell_new.subcell_loc from (protein_new left join experiment_new on protein_new.Experiment_idExperiment=experiment_new.idExperiment inner join subcell_new on protein_new.accession=subcell_new.accession) where protein_new.accession<>'NA' group by protein_new.accession") or die (mysql_error());
